...
Drop the folder in your personal skills directory:

```bash
cp -r ad-studio ~/.claude/skills/
```

Windows PowerShell:

```powershell
Copy-Item -Recurse ad-studio "$env:USERPROFILE\.claude\skills\"
```

Restart Claude Code. Type `/` — `ad-studio` should be in the list.

## Claude Code — for one project only

```bash
cp -r ad-studio /path/to/project/.claude/skills/
```
...
Zip the folder and upload it under **Settings → Features**. Requires a Pro, Max, Team or Enterprise plan with code execution enabled.
